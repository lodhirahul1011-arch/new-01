...
let RNQRGenerator: any = null;
try {
  RNQRGenerator = require('rn-qr-generator');
  if (RNQRGenerator && RNQRGenerator.default) {
    RNQRGenerator = RNQRGenerator.default;
  }
} catch {
  // rn-qr-generator is optional; live camera scanning is powered by react-native-vision-camera
}
...
export default function QrScannerScreen({ navigation, route }: Props) {
  ...
  const handleUploadQr = async () => {
    ...
    try {
      ...
      setState('processing');
      if (!RNQRGenerator?.detect) {
        logs.info('[qr-scanner] gallery QR detection unavailable');
        setErrorMessage('QR scanning from gallery is not supported. Please scan using the camera.');
        setState('error');
        return;
      }
